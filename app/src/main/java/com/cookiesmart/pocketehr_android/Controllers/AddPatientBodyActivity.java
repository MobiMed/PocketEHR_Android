...
//          bodyImage.setImageResource(R.drawable.male_body_parts);
    ...
    public void saveAndFinish(View v) {
        ...
        request.put("patientObjectId", visit.getPatient_object_id());
        request.put("eventType", visit.getClassType());
        request.put("eventStatus", visit.getStatusType());
        request.put("eventLocation", visit.getLocationType());
        request.put("initialObservation", visit.getInitialObservation());
        bodyParts.addAll(backBodyParts);
        try {
            JSONArray bodyPartsArray = new JSONArray(bodyParts.toArray());
            request.put("locations", bodyPartsArray);
        } catch (Exception e) {
            //
        }
